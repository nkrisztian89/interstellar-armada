...
function FVQ(model,shader,samplerName,cubemap,camera) {
	...
	this.uniformValueFunctions["u_viewDirectionProjectionInverse"] = function() { return Mat.inverse4(Mat.mul4(self.camera.getOrientationMatrix(),self.camera.getPerspectiveMatrix())); };
}
...
function Mesh(modelsWithLOD,shader,textures,positionMatrix,orientationMatrix,scalingMatrix,lineMode,smallestParentSizeWhenDrawn) {
	...
	this.uniformValueFunctions["u_modelMatrix"] = function() { return self.getCascadeModelMatrix(); };
	this.uniformValueFunctions["u_normalMatrix"] = function() { return Mat.transposed3(Mat.inverse3(Mat.matrix3from4(self.getCascadeModelMatrix()))); };
...
function Billboard(model,shader,texture,size,positionMatrix,orientationMatrix) {
	...
	this.uniformValueFunctions["u_modelMatrix"] = function() { return Mat.mul4(Mat.mul4(self.scalingMatrix,self.orientationMatrix),self.positionMatrix); };
        this.uniformValueFunctions["u_colorTexture"] = function() { return 0; };
}
...
function PointParticle(model,shader,color,positionMatrix) {
	...
	this.uniformValueFunctions["u_modelMatrix"] =   function() { return self.getCascadeModelMatrix(); };
        ...
        this.uniformValueFunctions["u_length"] =   function() { return Vec.length3(self.shift); };
        this.uniformValueFunctions["u_farthestZ"] =   function() { return 25.0; };
}
...
SceneCamera.prototype.setScene = function(scene) {
    !this._scene ?
        this._scene = scene :
        Armada.showError("Attempting to assign an already assigned camera to a different scene!","minor");
};
