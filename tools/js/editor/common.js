 ...
 * @param logic Used for obtaining environments and levels
 */
define([
    "utils/utils",
    ...
    "armada/classes",
    "armada/logic"
], function (utils, resources, classes, logic) {
    "use strict";
    /**
    ...
    function getItemReference(item) {
        switch (item.type) {
            ...
            case ItemType.ENVIRONMENT:
                return logic.getEnvironment(item.name);
            case ItemType.LEVEL:
                return {
                    getData: function () {
                        return null;
    ...
    function getItemReferencesOfSameCategory(item) {
        ...
        switch (item.type) {
            ...
            case ItemType.CLASS:
                ...
                break;
            case ItemType.ENVIRONMENT:
                names = logic.getEnvironmentNames();
                for (i = 0; i < names.length; i++) {
                    result.push(logic.getEnvironment(names[i]));
                }
                break;
            case ItemType.LEVEL:
                break;
